...
const Blog = ({ blog, setNewBlogs, user }) => {
    const blogStyle = {
        paddingTop: 10,
        paddingLeft: 2,
        border: 'solid',
    ...
    return (
        <div style={blogStyle}>
            <div className='initialContent' style={hideWhenView}>
                ...
                <button className='view-button' onClick={toggleView}>view</button>
            </div>
            <div className='moreContent' style={showWhenView}>
                {blog.title} {blog.author}&nbsp;
                <button className='hide-button' onClick={toggleView}>hide</button>
                <div>{blog.url}</div>
                <div>
                likes {blog.likes}&nbsp;
                ...
                {user.username === blog.user.username && (<button className='delete-button' onClick={deletePost}>remove</button>)}
            </div>
        </div>
    )}
